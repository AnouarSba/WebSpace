...
<div id="main" dir="rtl" style="width: 80%;">
  <h1 style="float: right;">تقييم المترشحين</h1>
  ...
  <table id="data" class="data-table data-table-horizontal data-table-highlight">
    <thead>
      <tr>
        ...
        <th style="text-align: center;"><table>
          <tr>
      <td style="width: 8%;">أأبعاد النقل و الجانب التنظيمي المتعلق بالنقل عبر الطرقات</td>
      <td style="width: 8%;">المفاهيم التقنية لمركبات النقل عبر الطرقات </td>
      <td style="width: 8%;">الوقاية و السلامة عبر الطرقات </td>

      <td style="width: 8%;">الاسعاف</td>
      <td style="width: 8%;">التنظيم المطبق على نقل الأشخاص غبر الطرقات</td>
      <td style="width: 8%;">فن حسن التصرف   </td>
      <td style="width: 8%;">سلوك السائق في مركز العمل </td>
      <td style="width: 8%;">تقنيات سياقة مركبة نقل الأشخاص عبر الطرقات</td>
    </tr>
   ...
   $result_c = $mysqli -> query("SELECT * FROM condidat_f where acc=1  and training=1  and id_f=".$id_f);
   $i =0;
...
$results = $mysqli -> query("SELECT * FROM client where id=".$row_c['id_c']);
   ...
   ";  $r1 = ($row_c["n1"] <21 )? $row_c["n1"] : "";
        ...
        $ch5 = ($row_c["n5"] <21 )? '' : "checked";
         ...
         $r6 = ($row_c["n6"] <21 )? $row_c["n6"] : "";
        $dis6 = ($row_c["n6"] <21 )? '' : "disabled";
        $ch6 = ($row_c["n6"] <21 )? '' : "checked";

         $r7 = ($row_c["n7"] <21 )? $row_c["n7"] : "";
        $dis7 = ($row_c["n7"] <21 )? '' : "disabled";
        $ch7 = ($row_c["n7"] <21 )? '' : "checked";

         $r8 = ($row_c["n8"] <21 )? $row_c["n8"] : "";
        $dis8 = ($row_c["n8"] <21 )? '' : "disabled";
        $ch8 = ($row_c["n8"] <21 )? '' : "checked";

         $r9 = ($row_c["n9"] <21 )? $row_c["n9"] : "";
        $dis9 = ($row_c["n9"] <21 )? '' : "disabled";
        $ch9 = ($row_c["n9"] <21 )? '' : "checked";
        $abs = ($row_c["n1"] ==21 || $row_c["n2"] ==21 || $row_c["n3"] ==21 || $row_c["n4"] ==21 || $row_c["n5"] ==21  || $row_c["n6"] ==21 || $row_c["n7"] ==21 || $row_c["n8"] ==21 || $row_c["n9"] ==21  )? 1 : 0;

     /*    if ($rows[8]==1) {
      $inp = ' &nbsp; <td '.$hidden.' class="note'.$i.'"><input type="number" style="width:100%"  step="0.25" id="ch9_'.$i.'" name="note9_'.$i.'" '.$dis9.' value="'.$r9.'" min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(9,'.$i.')" '.$ch9.' name="ch9_'.$i.'">غائب</td> ';
    } else*/ $inp = '';
      ...
      <td style="text-align: center;">'.$tr[$rows[8]].'</td>
        ...
        <table>
    <tr>
      <td '.$hidden.' class="note'.$i.'"><input type="number" style="width:100%"  step="0.25" id="ch1_'.$i.'" name="note1_'.$i.'" value="'.$r1.'" '.$dis1.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(1,'.$i.')" '.$ch1.' name="ch1_'.$i.'">غائب</td>

      <td '.$hidden.' class="note'.$i.'"><input type="number" style="width:100%"  step="0.25" id="ch2_'.$i.'" name="note2_'.$i.'" value="'.$r2.'" '.$dis2.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(2,'.$i.')" '.$ch2.' name="ch2_'.$i.'">غائب</td>

      <td '.$hidden.' class="note'.$i.'"> <input type="number" style="width:100%"  step="0.25" id="ch3_'.$i.'" name="note3_'.$i.'" value="'.$r3.'" '.$dis3.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(3,'.$i.')" '.$ch3.' name="ch3_'.$i.'">غائب</td>

      <td '.$hidden.' class="note'.$i.'"> <input type="number" style="width:100%"  step="0.25" id="ch4_'.$i.'" name="note4_'.$i.'" value="'.$r4.'" '.$dis4.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(4,'.$i.')" '.$ch4.' name="ch4_'.$i.'">غائب</td>
      <td '.$hidden.' class="note'.$i.'"><input type="number" style="width:100%"  step="0.25" id="ch5_'.$i.'" name="note5_'.$i.'" value="'.$r5.'" '.$dis5.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(5,'.$i.')" '.$ch5.' name="ch5_'.$i.'">غائب</td>

      <td '.$hidden.' class="note'.$i.'"><input type="number" style="width:100%"  step="0.25" id="ch6_'.$i.'" name="note6_'.$i.'" value="'.$r6.'" '.$dis6.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(6,'.$i.')" '.$ch6.' name="ch6_'.$i.'">غائب</td>

      <td '.$hidden.' class="note'.$i.'"><input type="number" style="width:100%"  step="0.25" id="ch7_'.$i.'" name="note7_'.$i.'" value="'.$r7.'" '.$dis7.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(7,'.$i.')" '.$ch7.' name="ch7_'.$i.'">غائب</td>

       <td '.$hidden.' class="note'.$i.'"><input type="number" style="width:100%"  step="0.25" id="ch8_'.$i.'" name="note8_'.$i.'" value="'.$r8.'" '.$dis8.' min=0 max=20 style="width:140px" /><br>
      <input type="checkbox" onclick="checkk(8,'.$i.')" '.$ch8.' name="ch8_'.$i.'">غائب</td>
    ...
    <thead>
      <tr>
        ...
        <th style="text-align: center;"><table>
          <tr>
      <td style="width: 8%;">أبعاد النقل و الجانب التنظيمي المتعلق بالنقل عبر الطرقات</td>
      <td style="width: 8%;">المفاهيم التقنية لمركبات النقل عبر الطرقات </td>
      <td style="width: 8%;">الوقاية و السلامة عبر الطرقات </td>

      <td style="width: 8%;">الاسعاف</td>
      <td style="width: 8%;">التنظيم المتعلق بنقل البضائع عبر الطرقات</td>
      <td style="width: 8%;">القواعد التقنية للسياقة   </td>
      <td style="width: 8%;">فن القيادة الجيدة</td>
      <td style="width: 8%;">: تقنيات سياقة مركبة الوزن الثقيل لنقل البضائع عبر الطرقات</td>
    </tr>
   ...
   $result_c = $mysqli -> query("SELECT * FROM condidat_f where acc=1  and training=2  and id_f=".$id_f);
while ($row_c = $result_c->fetch_assoc()) {
...
$results = $mysqli -> query("SELECT * FROM client where id=".$row_c['id_c']);
  ...
  echo " <input type='hidden'  name='id' id='f".$id_f."' value='".$id_f."'><input type='hidden' name='f_ar' id='f_ar".$id_f."'><input type='hidden' name='f_num_ar_f' id='f_ar_f".$id_f."'> "; ?>
    ...
    <input type="hidden" name="ar" class="form-control text-right" id="ar" placeholder="مدة التكوين"  >
